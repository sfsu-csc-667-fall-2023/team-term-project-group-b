...
const handler = async (request, response) => {
  const io = request.app.get("io");
    ...
    if(!isValidBet){
        const message = "Not enough chips to place bet"
        io.to(game_socket_id).emit(GAME_CONSTANTS.ERROR_NOT_ENOUGH_CHIPS, {message: message})
    ...
    if(isPlayerInGame && isPlayerTurn){
      ...
      const gameState = Games.getState(gameId);
      

      //check round

      ///// 
      io.to(game_socket_id).emit(GAME_CONSTANTS.UPDATE_CURRENT_TURN, 
